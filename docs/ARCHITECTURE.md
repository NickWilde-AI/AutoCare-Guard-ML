...
智能汽车服务风险决策平台（AutoCare Risk Intelligence Platform）是面向新能源汽车售后风险智能研判与工单路由的企业级 AI/ML 工程框架。代码与模型仓库为 AutoCare-Guard-ML。它将模型研判、输出校验、工单路由、可观测性和样本回流拆成相互独立但可组合的模块。
