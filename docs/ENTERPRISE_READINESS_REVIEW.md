...
当前项目（智能汽车服务风险决策平台 / AutoCare Risk Intelligence Platform）提供售后风险研判与工单路由系统的脱敏工程骨架：多证据输入、结构化输出、解析兜底、策略路由、评测、监控、告警、版本追踪、API 服务、部署模板、契约门禁、环境 preflight、模型注册表示例和单元测试。公开实现适合技术交流和方案评审，不代表完整生产系统。
