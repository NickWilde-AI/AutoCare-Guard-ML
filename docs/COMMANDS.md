...
```bash
PYTHONPATH=src python3 -m autocare_guard_ml.cli --config configs/default.yaml summary data/local/input.jsonl

PYTHONPATH=src python3 -m autocare_guard_ml.cli --config configs/default.yaml predict data/local/input.jsonl \
  --with-route \
  --with-version \
  --audit-log-out outputs/demo_audit_logs.jsonl \
  --out outputs/demo_routed_predictions.jsonl

PYTHONPATH=src python3 -m autocare_guard_ml.cli --config configs/default.yaml eval outputs/demo_routed_predictions.jsonl
PYTHONPATH=src python3 -m autocare_guard_ml.cli --config configs/default.yaml eval-report outputs/demo_routed_predictions.jsonl --out outputs/offline_eval_report.md
PYTHONPATH=src python3 -m autocare_guard_ml.cli --config configs/default.yaml delivery-summary --out outputs/enterprise_delivery_summary.md
PYTHONPATH=src python3 -m autocare_guard_ml.cli --config configs/default.yaml readiness-check --out outputs/readiness_check.json
PYTHONPATH=src python3 -m autocare_guard_ml.cli --config configs/default.yaml monitor outputs/demo_routed_predictions.jsonl
PYTHONPATH=src python3 -m autocare_guard_ml.cli --config configs/default.yaml alerts outputs/demo_routed_predictions.jsonl
PYTHONPATH=src python3 -m autocare_guard_ml.cli --config configs/default.yaml window-alerts outputs/demo_routed_predictions.jsonl --window-size 100 --step-size 50
PYTHONPATH=src python3 -m autocare_guard_ml.cli --config configs/default.yaml drift-report outputs/demo_routed_predictions.jsonl --baseline-pred-jsonl outputs/demo_routed_predictions.jsonl --out outputs/drift_report.json
PYTHONPATH=src python3 -m autocare_guard_ml.cli --config configs/default.yaml ab-report --control outputs/demo_routed_predictions.jsonl --candidate outputs/demo_routed_predictions.jsonl --out outputs/ab_report.md --json-out outputs/ab_report.json
PYTHONPATH=src python3 -m autocare_guard_ml.cli --config configs/default.yaml api-contract --out outputs/openapi_contract.json --fail-on-missing
PYTHONPATH=src python3 -m autocare_guard_ml.cli --config configs/default.yaml production-preflight --env-file deploy/audit_service.prod.env.example --out outputs/production_preflight.json
PYTHONPATH=src python3 -m autocare_guard_ml.cli --config configs/default.yaml model-registry-check --registry configs/model_registry.yaml --out outputs/model_registry_check.json
PYTHONPATH=src python3 -m autocare_guard_ml.cli --config configs/default.yaml audit-data data/local/input.jsonl
```
...
```bash
pip install -e ".[train]"
autocare-guard --config configs/default.yaml train data/train/autocare_train.jsonl
```
...
```bash
pip install -e ".[serve]"
autocare-guard --config configs/default.yaml serve --port 8000
```
...
按 `case_id` 查询 API 审计事件（路径参数名仍为 `ticket_id`，值可用 `case_id`）：
...
```bash
export AUTOCARE_GUARD_API_TOKEN_HASHES="<writer-token-64-hex-sha256>:writer,<reader-token-64-hex-sha256>:reader,<audit-token-64-hex-sha256>:auditor"
```
...
PYTHONPATH=src autocare-guard --config configs/local_mps_train.yaml train data/train/xguard_splits/train.jsonl
```
...
```bash
LC_ALL=en_US.UTF-8 LANG=en_US.UTF-8 \
PYTHONPATH=src autocare-guard --config configs/local_fast_full_train.yaml train data/train/xguard_splits/train.jsonl
```
...
如果开启了 `AUTOCARE_GUARD_API_TOKEN`：
